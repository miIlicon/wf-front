...
import { InputProps, SearchResultProps } from "../../@types/typs";
...
const Input = ({ onChange, value }: InputProps) => {
  return (
    <div>
      ...
      { value.length > 0 && <ResultModal /> }
    </div>
  )
}

const Result = ({ title, subTitle, thumb, state } : SearchResultProps) => {
  return (
    <div
      css={css`
        font-family: "Pretendard-Medium";
    ...
    >
      ...
      >
        ...
        <EventStatusButton status={state} />
      </div>
    </div> 
  )
}
